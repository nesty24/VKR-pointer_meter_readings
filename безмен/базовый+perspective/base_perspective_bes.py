...
def _score_contour(cnt, h, w, short, prev_ellipse):
    ...
    if area < max(6500.0, 0.012 * h * w) or len(cnt) < 5:
        return None, -1.0
    ...
    if ratio < 0.40:
        return None, -1.0
    if major < short * 0.26 or major > short * 0.95:
        return None, -1.0
    ...
    if prev_ellipse is not None:
        ...
        score += max(0.0, 1.0 - dist_c / 220.0) * 0.25
        score += max(0.0, 1.0 - dist_a / 220.0) * 0.15
...
def _stabilize_ellipse(curr_ellipse, prev_ellipse):
    ...
    if center_jump > 95.0 or major_rel > 0.30 or minor_rel > 0.30:
        return curr_ellipse

    if center_jump < 15.0:
        alpha = 0.55
    elif center_jump < 35.0:
        alpha = 0.35
    else:
        alpha = 0.15
        ...
        if idx % args.skip == 0:
            ...
            if ellipse is not None:
                ...
                if args.debug_dir and (idx % args.debug_every == 0):
                    angle, pending_debug_data = find_needle_angle(warped, wcx, wcy, wr, return_debug=True)
                else:
...
def main():
    p = argparse.ArgumentParser(
        description="Распознавание показаний безмена по видео (с коррекцией перспективы)")
