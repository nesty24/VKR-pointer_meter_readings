...
def find_needle_angle(frame, cx, cy, r, return_debug=False):
    h, w = frame.shape[:2]
    pad = int(r * 1.05)
    ...
    cv2.circle(ring_mask, (lx, ly), int(r * 0.82), 255, -1)
    cv2.circle(ring_mask, (lx, ly), int(r * 0.16), 0, -1)
    ...
    if return_debug:
        line_len = int(max(crop.shape[0], crop.shape[1]) * 0.7)
        p1 = (int(x0 - vx * line_len), int(y0 - vy * line_len))
        p2 = (int(x0 + vx * line_len), int(y0 + vy * line_len))
        debug_data["contour"] = best_cnt.copy()
        debug_data["line"] = (p1, p2)
        debug_data["angle"] = angle
        return angle, debug_data

    return angle


def save_needle_debug(debug_dir, frame_idx, debug_data):
    if not debug_data:
        return
    os.makedirs(debug_dir, exist_ok=True)

    crop_vis = debug_data["crop"].copy()
    lx, ly = debug_data["center"]
    cv2.circle(crop_vis, (lx, ly), 3, (0, 255, 255), -1)

    status = "miss"
    if debug_data["contour"] is not None:
        cv2.drawContours(crop_vis, [debug_data["contour"]], -1, (0, 255, 0), 2)
        status = "ok"
    if debug_data["line"] is not None:
        p1, p2 = debug_data["line"]
        cv2.line(crop_vis, p1, p2, (0, 0, 255), 2)

    if not math.isnan(debug_data["angle"]):
        cv2.putText(crop_vis, f"ang={debug_data['angle']:.2f}",
                    (8, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 0, 255), 2, cv2.LINE_AA)

    base = f"frame_{frame_idx:06d}_{status}"
    cv2.imwrite(os.path.join(debug_dir, base + "_crop.png"), crop_vis)
    cv2.imwrite(os.path.join(debug_dir, base + "_bin.png"), debug_data["binary"])
...
def smooth_value(buf, ema, value, alpha=0.35):
    if math.isnan(value):
        return buf, ema, (ema if ema is not None else float('nan'))
    buf.append(value)
    med = float(np.median(list(buf)))
    ema = med if ema is None else alpha * med + (1 - alpha) * ema
    return buf, ema, ema
    ...
    while True:
        ...
        if idx % args.skip == 0:
            ...
            if gauge:
                prev_gauge = gauge
                cx, cy, r = gauge
                if args.debug_dir and (idx % args.debug_every == 0):
                    angle, pending_debug_data = find_needle_angle(frame, cx, cy, r, return_debug=True)
                else:
                    angle = find_needle_angle(frame, cx, cy, r)
                raw = angle_to_value(angle, args.start_angle, args.end_angle, args.min_val, args.max_val)
                smooth_buf, smooth_ema, last_val = smooth_value(smooth_buf, smooth_ema, raw)
            else:
                smooth_buf, smooth_ema, last_val = smooth_value(smooth_buf, smooth_ema, float('nan'))

            algo_time_sum += time.perf_counter() - t_alg_start
            processed_frames += 1

            if pending_debug_data is not None:
                save_needle_debug(args.debug_dir, idx, pending_debug_data)
...
def main():
    p = argparse.ArgumentParser(
        description="Распознавание показаний безмена по видео (сглаживание)")
    ...
    p.add_argument("--debug-dir", help="Папка для debug-кадров найденной стрелки")
    p.add_argument("--debug-every", type=int, default=1,
                   help="Сохранять debug каждый N-й обрабатываемый кадр [1]")
