...
def find_needle_angle(frame, cx, cy, r):
    ...
    cv2.circle(ring_mask, (lx, ly), int(r * 0.14), 0, -1)

    mean_brightness = cv2.mean(gray, mask=ring_mask)[0]
    masked_pixels = gray[ring_mask > 0]
    ...
    if mean_brightness < 128:
        adaptive = cv2.adaptiveThreshold(
            ...
            cv2.THRESH_BINARY,
            blockSize=51, C=-8)
        global_gate = (gray > np.percentile(masked_pixels, 80)).astype(np.uint8) * 255
    else:
        adaptive = cv2.adaptiveThreshold(
            gray_blur, 255,
            cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
            ...
            blockSize=51, C=6)
        global_gate = (gray < np.percentile(masked_pixels, 22)).astype(np.uint8) * 255

    binary = cv2.bitwise_and(adaptive, global_gate)
...
def main():
    p = argparse.ArgumentParser(
        description="Распознавание показаний безмена по видео (комбинированная бин.)")
