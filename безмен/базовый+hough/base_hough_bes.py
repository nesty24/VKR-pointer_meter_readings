...
def find_needle_angle(frame, cx, cy, r):
    ...
    lines = cv2.HoughLinesP(
        ...
        1,
        np.pi / 180,
        threshold=max(14, int(r * 0.07)),
        minLineLength=int(r * 0.07),
        maxLineGap=max(8, int(r * 0.06)),
    )
    if lines is None:
        return float("nan")
    ...
    for ln in lines:
        ...
        da = math.hypot(ax - lx, ay - ly)
        db = math.hypot(bx - lx, by - ly)
        near_r = min(da, db)
        far_r = max(da, db)

        if near_r > r * 0.46:
            continue
        if far_r < r * 0.27 or far_r > r * 0.88:
            continue
        if (far_r - near_r) < r * 0.16:
            continue

        denom = dx * dx + dy * dy
        if denom == 0:
            continue
        t = max(0, min(1, ((lx - ax) * dx + (ly - ay) * dy) / denom))
        px, py = ax + t * dx, ay + t * dy
        dist_to_center = math.hypot(px - lx, py - ly)
        if dist_to_center > r * 0.17:
            continue

        score = far_r + 0.75 * length - 0.70 * near_r - 0.90 * dist_to_center
        if score > best_score:
            best_score = score
            if da > db:
                tip_dx, tip_dy = ax - lx, ay - ly
            else:
                tip_dx, tip_dy = bx - lx, by - ly
            best_angle = math.degrees(math.atan2(-tip_dy, tip_dx))
...
def main():
    p = argparse.ArgumentParser(
        description="Распознавание показаний безмена по видео (линии Хафа)"
    )
    ...
    p.add_argument("--skip", type=int, default=3, help="Обрабатывать каждый N-й кадр [3]")

    g = p.add_argument_group("Калибровка (все параметры обязательные)")
