...
def find_needle_angle(frame, cx, cy, r):
    ...
    binary_soft = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, k, iterations=1)
    binary_strict = cv2.morphologyEx(binary_soft, cv2.MORPH_OPEN, k, iterations=3)

    lines = cv2.HoughLinesP(
        binary_strict,
        1,
        np.pi / 180,
        threshold=max(18, int(r * 0.08)),
        minLineLength=int(r * 0.24),
        maxLineGap=max(6, int(r * 0.03)),
    )
    if lines is None:
        return float("nan")
    ...
    for ln in lines:
        ...
        if dist_to_center > r * 0.12:
            continue

        score = far_r + 0.80 * length - 1.20 * near_r - 1.20 * dist_to_center
        if score > best_score:
            best_score = score
            if da > db:
...
def main():
    p = argparse.ArgumentParser(
        description="Распознавание показаний индикатора по видео (линии Хафа)"
    )
