...
def find_needle_angle(frame, cx, cy, r):
    ...
    cv2.circle(ring_mask, (lx, ly), int(r * 0.72), 255, -1)
    cv2.circle(ring_mask, (lx, ly), int(r * 0.08), 0, -1)
    ...
    if mean_brightness < 128:
        ...
        adaptive = cv2.adaptiveThreshold(
            ...
            blockSize=41, C=2)
        global_gate = (gray < np.percentile(masked_pixels, 36)).astype(np.uint8) * 255
    ...
    for cnt in contours:
        ...
        if near_r > r * 0.21 or far_r < r * 0.42:
            continue
...
def main():
    p = argparse.ArgumentParser(
        description="Распознавание показаний индикатора по видео (комбинированная бин.)")
