...
# ==========================
GT_FILE = r"C:\Users\79213\PycharmProjects\python\диплом\протестированное\спидометр\ручная разметка\top_hand.csv"
P15_FILE = r"C:\Users\79213\PycharmProjects\python\диплом\протестированное\спидометр\базовый+smooth\top_smooth (vs human).csv"
...
def calculate_metrics(gt_path: Path, pred_path: Path) -> int:
    ...
    used_rows: List[Tuple[int, int, int, int]] = []
    ...
    for line_no, (gt_line, pred_line) in enumerate(
        ...
        gt_value = round(gt_value_raw)
        pred_value = round(pred_value_raw)
        abs_error = abs(pred_value - gt_value)

        used_rows.append((gt_frame, pred_value, gt_value, abs_error))
    ...
    print()
    print("e_i = |p_i - g_i| по каждому использованному кадру:")
    for frame, pred_value, gt_value, abs_error in used_rows:
        print(
            f"  кадр {frame:5d}: "
            f"p_i={pred_value:4d}, g_i={gt_value:4d}, "
            f"e_i=|{pred_value} - {gt_value}|={abs_error}"
        )
    ...
    print()
    print(f"MAE = (1/n) * sum(e_i) = {mae:.4f}")
    print()
    print(f"Отсортированные ошибки для Median AE и P95: {sorted_errors}")
    ...
    print()
    print(f"Сравнение |p_i - g_i| с threshold={ACC_THRESHOLD}:")
    for frame, _, _, abs_error in used_rows:
        status = "OK" if abs_error <= ACC_THRESHOLD else "FAIL"
        sign = "<=" if abs_error <= ACC_THRESHOLD else ">"
        print(
            f"  кадр {frame:5d}: |p_i - g_i| = {abs_error:3d} {sign} {ACC_THRESHOLD} -> {status}"
        )

    print()
    print(
        "Acc = (кадры где |p_i - g_i| <= threshold) / n * 100% = "
    ...
    print(f"MAE     = (1/n) * sum(e_i)                     = (1/{n}) * {sum(errors):.4f} = {mae:.4f}")
    print(f"MedianAE= median(sorted(|p_i - g_i|))          = {median_ae:.4f}")
    print(f"nMAE    = MAE / R * 100%                       = {mae:.4f} / {SCALE_RANGE_R} * 100% = {nmae:.4f}%")
    print(f"P95     = percentile_95(sorted(|p_i - g_i|))   = {p95:.4f}")
    print(
        f"Acc     = count(|p_i - g_i| <= threshold) / n * 100% "
