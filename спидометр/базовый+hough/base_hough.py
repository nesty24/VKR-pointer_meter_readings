...
def find_needle_angle(frame, cx, cy, r):
    ...
    lines = cv2.HoughLinesP(binary, 1, np.pi / 180, threshold=25, minLineLength=int(r * 0.25), maxLineGap=10)
    if lines is None:
        return float('nan')

    best_score = -1
    best_angle = float('nan')
    max_dist_to_center = r * 0.15
    ...
    for ln in lines:
        ...
        dx, dy = bx - ax, by - ay
        t = max(0, min(1, ((lx - ax) * dx + (ly - ay) * dy) / (dx*dx + dy*dy)))
        px, py = ax + t * dx, ay + t * dy
        dist_to_center = math.hypot(px - lx, py - ly)

        if dist_to_center > max_dist_to_center:
            continue

        score = length / max(dist_to_center, 1)
        if score > best_score:
            best_score = score
...
def main():
    p = argparse.ArgumentParser(
        description="Распознавание показаний спидометра по видео (линии Хафа)")
